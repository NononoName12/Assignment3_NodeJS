  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(dataJson.prods); // Log ra dữ liệu phản hồi từ API

        setProduct(dataJson.prods); // Cập nhật state với danh sách sản phẩm
      } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await response.json(); // Chuyển dữ liệu response sang JSON
        setDetail(data.prods);
      } catch (error) {
